...
// Number of Leds
  ...
  const int NUM_LEDSB = 118; // Back strip LED_COUNT   // Change to (final led count) on the finished version
...
void meltTip(bool on) { // Changes the tipleds to the tipmelt color if called with true, changes them back if called with false.
  ...
  if (on) {// Changes the tip to Tipmelt color.
    ...
    // *** Debugging ***
      ...
      for(int j=NUM_LEDSF-1; j>NUM_LEDSF - tipSize; j--) { // Turns on the first half of the tipmelt leds in the tipmelt color.
    ...
    // *** Debugging ***
      ...
      for(int j=NUM_LEDSF; j>NUM_LEDSF - tipSize; j--) {
        ledsF[j].setRGB( tipRed, tipGreen, tipBlue);
        ledsB[j].setRGB( tipRed, tipGreen, tipBlue);
      }
